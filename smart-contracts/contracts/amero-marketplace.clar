...
(use-trait comission-trait .comission-trait.commission)
...
(define-constant no-agent (err u2009))
...
(define-map listings
	uint
	{
		...
		agent_principal: (optional principal)
	}
)
...
(define-public (list-asset (nft-asset-contract <nft-trait>) (nft-asset {taker: (optional principal), token-id: uint, expiry: uint, price: uint, payment-asset-contract: (optional principal), agent_principal: (optional principal)}))
...
(define-private (assert-can-fulfil (nft-asset-contract principal) (payment-asset-contract (optional principal)) (listing {maker: principal, taker: (optional principal), token-id: uint, nft-asset-contract: principal, expiry: uint, price: uint, payment-asset-contract: (optional principal), agent_principal: (optional principal)}))
	(begin
...
(define-public (fulfil-listing-stx (listing-id uint) (nft-asset-contract <nft-trait>) (comm <comission-trait>))
	(let (
		(listing (unwrap! (map-get? listings listing-id) err-unknown-listing))
		(taker tx-sender)
		(id (get token-id listing))
		(price (get price listing))
		(agent (unwrap! (get agent_principal listing) no-agent))
		)
		(try! (assert-can-fulfil (contract-of nft-asset-contract) none listing))
		(try! (as-contract (transfer-nft nft-asset-contract (get token-id listing) tx-sender taker)))
		(try! (stx-transfer? (get price listing) taker (get maker listing)))
		(try! (contract-call? comm pay id price agent))
		(map-delete listings listing-id)
		(ok listing-id)
	)
)

(define-public (fulfil-listing-ft (listing-id uint) (nft-asset-contract <nft-trait>) (payment-asset-contract <ft-trait>) (comm <comission-trait>))
	(let (
		(listing (unwrap! (map-get? listings listing-id) err-unknown-listing))
		(taker tx-sender)
		(id (get token-id listing))
