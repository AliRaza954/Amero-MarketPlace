...
import { myStxAddress } from "../stacks-calls/auth";
// import { userSession } from "./_app";
import { userSession } from "../stacks-calls/auth";
...
export default function Home() {
  ...
  useEffect(() => {
    const get_nft_data = async() => {
      const nft_data = await fetchNFTData(userSession);
    ...
    <div>
      ...
      <div>
        ...
        <Row>
          ...
          {data.map((nft) => {
            console.log(nft[0].id)
            return(
            <Col sm="6" lg="6" xl="3" key={nft[0].id}>
            <Blog
              image={nft[0].URL}
              id = {nft[0].id}
            />
